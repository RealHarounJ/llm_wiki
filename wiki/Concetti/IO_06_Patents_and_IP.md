...
- **R&D (Input)**: Frascati manual. Easy to measure monetarily. Ignores failures and non-tech innovation. Biased towards large firms.
- **Innovation Surveys (Outcome)**: CIS. Captures non-tech innovation and SMEs, but relies on subjective self-assessment and suffers from low response rates.
- **Patents (Intermediate Output)**: Exclusive rights for 20 years. 
  - *Requirements*: Novelty, Non-obviousness (Inventive step), Utility (Industrial application).

## AI Patents
Analyzed across three dimensions:
1. **Techniques** (HOW): Machine Learning, Fuzzy Logic.
2. **Functional Applications** (WHAT): Computer Vision (dominates at 49%), NLP.
3. **Application Fields** (WHERE): Telecom, Transportation, Healthcare.
